...
public abstract class RippleView extends CustomView{
	...
	protected void setRippleAttributes(AttributeSet attrs) {
		 ...
		 * Color by resource
		 */
		int color = attrs.getAttributeResourceValue(MATERIALDESIGNXML,"rippleColors",-1);
		if(color != -1){
			rippleColor = getResources().getColor(color);
			settedRippleColor = true;
		}else{
			// Color by hexadecimal
			int rColor = attrs.getAttributeIntValue(MATERIALDESIGNXML, "rippleColors", -1);// 16进制的颜色
